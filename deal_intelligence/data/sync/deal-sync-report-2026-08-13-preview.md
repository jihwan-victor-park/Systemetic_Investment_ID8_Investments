# Attio <-> Hub deal sync -- 2026-08-13-preview

- Attio companies (deals grouped by company): **302**
- Hub companies (Firestore `companies`, round docs folded in): **310**
- Matched on both sides: **302** (302 by key)
- **In Attio, missing from the hub: 0**
- **In the hub, missing from Attio: 8**
- Placement disagreements among matched deals: **37** (agreed: 128)
- Matched but filed by hand in Attio (Passed/Invested -- rule not applied): 35
- Matched but the rule places them nowhere: 102 (16 above B with no Tier 1 (33), 86 below/unknown series)

Those five buckets partition the 302 matched companies (37 + 128 + 35 + 16 + 86 = 302), so no matched deal is missing from the report.
...
0 companies. `Should be` is where the rule puts them; a blank means the rule places them nowhere (below mandate, or no Tier 1 backer) -- those need a human call, not an automatic push.
...
| Simile | Series B | Radar | pipeline +qualified | Qualified + radar | +tag radar | stage -> Qualified | Series B with a Top 10 investor (in mandate at B+, and just raised) |
...
16 matched companies clear the B+ mandate but have no Tier 1 (33) firm on the cap table, so the Qualified clause of the rule does not fire. The live intake path files these as Qualified anyway (`determine_placement` never checks Tier 1) -- this is the population affected by that difference.
...
86 companies below Series B without a Top 10 backer, or with no usable series on file. They stay wherever they are today -- listed so the numbers above reconcile and so a wrong/blank Series is visible as the cause.

- Series B with a Tier 1 (33) but no Top 10 investor: **40**
...
| Hadrian | Series B | Pipeline | pipeline | Series B with a Tier 1 (33) but no Top 10 investor |
...
57 companies sit in the one band the rule reads two ways. Under `dual` (the default, and what the shipped intake code does) they are Qualified in Attio and additionally tagged `radar` in the hub. Under `radar` they would be Radar only. Rerun with `--series-b-mode radar` to see that version.

| Company | Series | Attio | Hub | Top 10 |
|---|---|---|---|---|
| Actively AI | Series B | Target | new | Bain Capital Ventures |
| Advanced Manufacturing Company of America | Series B | Qualified | qualified +radar | Lightspeed Venture Partners |
...
| Bezi | Series B | Passed | new +passed | Bessemer Venture Partners |
| Black Forest Labs | Series B | Qualified | qualified +radar | Bain Capital Ventures |
| Braintrust (Software Development Applications) | Series B | Qualified | qualified +radar | ICONIQ Capital |
...
| PermitFlow | Series B | Passed | new +passed | Accel |
...
| Simile | Series B | Radar | pipeline +qualified | Index Ventures, Bain Capital Ventures, Greenoaks Capital Partners |
...
| Upwind Security | Series B | Passed | new +passed | Sequoia Capital, Bessemer Venture Partners |
...
35 matched companies carry a stage the series rule must not override. Listed so nothing is invisible; no action proposed.

Airwallex (Passed), Assort Health (Passed), Bedrock Robotics (Passed), Bezi (Passed), ChipAgents (Passed), Cloaked (Passed), Commure (Passed), Coretsu (Logical Intelligence) (Passed), Crusoe Energy (Passed), Decart.ai (Passed), Erebor (Passed), Eudia (Passed), Firestorm (Passed), Hayden AI (Passed), Inspiren (Passed), Klir (Passed), Letter AI (Passed), Maybern (Passed), Northwood (Passed), Parloa (Passed), Pepper (Passed), Periodic Labs (Passed), PermitFlow (Passed), Polymarket (Invested), Radiant (Passed), Reflection AI (Invested), Saronic (Invested), Skild AI (Passed), Sprouts.ai (Passed), Stacklet (Passed), Standard Bots (Passed), Stepful (Passed), Upwind Security (Passed), Varda Space (Passed), Vatn (Passed)
