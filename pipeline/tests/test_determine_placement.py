...
def test_below_b_routes_to_radar(series):
    assert determine_placement(series, "Qualified") == ("Radar", ["radar"])
    # Overrides a Watchlist default too -- the series rule wins.
    assert determine_placement(series, "Watchlist") == ("Radar", ["radar"])
...
def test_series_b_lands_in_both_buckets(series):
    ...
    # pipeline view), with the hub carrying the dual truth via tags.
    assert stage == "Qualified"
    assert sorted(tags) == ["qualified", "radar"]
...
def test_series_b_keeps_a_watchlist_default_as_its_attio_stage():
    ...
    assert sorted(tags) == ["radar", "watchlist"]


def test_series_b_does_not_duplicate_the_radar_tag():
    # A caller whose default already IS Radar must not produce ['radar','radar'].
    assert determine_placement("Series B", "Radar") == ("Radar", ["radar"])
...
def test_above_b_uses_the_default_stage(series):
    assert determine_placement(series, "Qualified") == ("Qualified", ["qualified"])
    assert determine_placement(series, "Watchlist") == ("Watchlist", ["watchlist"])
