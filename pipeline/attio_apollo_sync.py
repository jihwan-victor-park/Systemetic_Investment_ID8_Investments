...
ENROLL_CHUNK = 100  # add_contact_ids accepts an array; chunk to stay reliable


def _enroll_chunk(contact_ids):
    """Add up to ENROLL_CHUNK contacts to the holding sequence in one call.

    Re-run safe: a contact can only be a member of a given sequence once, so
    re-sending an already-enrolled contact is a no-op on Apollo's side (no
    duplicate membership). run_dedupe on the contact upsert prevents duplicate
    contacts, so the whole weekly sync is idempotent.

    sequence_active_in_other_campaigns / sequence_finished_in_other_campaigns are
    left False on purpose: contacts already live in a REAL outreach sequence are
    skipped (we don't disturb them — they're already flagged in prospecting
    searches anyway). NOTE: Apollo's docs use these snake_case names and require a
    MASTER api key for this endpoint (403 otherwise).
    """
    payload = {
        "emailer_campaign_id":                  APOLLO_HOLDING_SEQUENCE_ID,
        "contact_ids":                          contact_ids,
        "sequence_active_in_other_campaigns":   False,
        "sequence_finished_in_other_campaigns": False,
    }
    if APOLLO_MAILBOX_ID:
        payload["send_email_from_email_account_id"] = APOLLO_MAILBOX_ID
    resp = requests.post(
    ...
    # Response echoes the contacts it actually enrolled this call; already-enrolled
    # or skipped (active elsewhere) contacts are simply absent — not an error.
    body = resp.json()
    added = body.get("contacts") or body.get("emailer_campaign", {}).get("contacts") or []
    return (len(added) if isinstance(added, list) else len(contact_ids)), None


def enroll_in_holding_sequence(contact_ids):
    """Enroll all contacts in the holding sequence, chunked. Returns (added, errors)."""
    if not APOLLO_HOLDING_SEQUENCE_ID:
        return 0, ["APOLLO_HOLDING_SEQUENCE_ID not set"]
    # de-dupe ids (two Attio people can resolve to the same Apollo contact)
    ids = list(dict.fromkeys(i for i in contact_ids if i))
    added, errors = 0, []
    for i in range(0, len(ids), ENROLL_CHUNK):
        chunk = ids[i:i + ENROLL_CHUNK]
        try:
            n, err = _enroll_chunk(chunk)
            added += n
            if err:
                errors.append(err)
        except Exception as e:
...
def run():
    ...
    if apollo_ids and APOLLO_HOLDING_SEQUENCE_ID:
        ...
        enrolled, errs = enroll_in_holding_sequence(apollo_ids)
